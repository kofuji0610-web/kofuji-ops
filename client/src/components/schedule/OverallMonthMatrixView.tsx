...
import type { OverallWeekMatrixMember, OverallWeekMatrixSchedule } from "./OverallWeekMatrixView";

/** スケジュール画面「全体共有 × 月表示」用マトリクス（列＝当月の全日） */
...
export type OverallMonthMatrixViewProps = {
...
export function OverallMonthMatrixView({
  schedules,
  monthDays,
  members,
  activeDepts,
  onEventClick,
}: OverallMonthMatrixViewProps) {
  ...
  const gridCols = useMemo(
    () => `minmax(5rem, 6rem) repeat(${monthDays.length}, minmax(0, 1fr))`,
    [monthDays.length]
  );
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-y-auto rounded-md border border-slate-200 bg-white [scrollbar-width:thin]">
      ...
      >
        <div className="sticky top-0 z-10 bg-slate-100 px-1 py-1 font-semibold text-slate-700"> </div>
        {monthDays.map((d) => {
          ...
          return (
            <div
              key={ymdH}
              className={cn(
                "sticky top-0 z-10 px-0.5 py-0.5 text-center font-medium text-[10px] text-slate-600",
                hol && "bg-pink-50",
                !hol && sat && "bg-sky-50",
            ...
            >
              <div className="text-[9px] text-muted-foreground leading-none">{wd}</div>
              <div className={cn("text-[11px] font-semibold leading-tight", isToday && "text-primary")}>
                {d.getDate()}
              </div>
            </div>
          );
            ...
            row.kind === "member" ? (
              ...
              <span className="flex items-center gap-2 font-semibold text-slate-800">
                {(() => {
                  const Icon = DEPT_ICON_MAP[row.key];
                  return (
                    <Icon className="h-4 w-4 shrink-0" style={{ color: DEPT_COLOR_MAP[row.key] }} aria-hidden />
                  );
                })()}
                {labelForRow(row)}
              </span>
